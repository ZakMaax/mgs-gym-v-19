...
class SalesReportExcelController(http.Controller):
    ...
    def export_sales_report_excel(
        ...
        sheet.merge_range(
            "A1:J1", company, workbook.add_format({"bold": True, "font_size": 14})
        )
        sheet.merge_range(
            "A2:J2", title, workbook.add_format({"bold": True, "font_size": 12})
        )
        sheet.merge_range(
            "A3:J3", date_range, workbook.add_format({"italic": True, "font_size": 10})
        )
        row = 5
        ...
        if report_type == "detail":
            for section in data:
                ...
                for line in section["orders"]:
                    ...
                    sheet.write_number(row, 7, line["amount"], number_format)
                    sheet.write_number(row, 8, line["to_invoice_qty"], number_format)
                    sheet.write_number(row, 9, line["invoiced_qty"], number_format)

                    # Update group totals
                    group_totals["amount"] += line["amount"]
                    group_totals["ordered"] += line["ordered_qty"]
                    group_totals["delivered"] += line["delivered_qty"]
                    group_totals["to_invoice"] += line["to_invoice_qty"]
                ...
                sheet.write_number(row, 7, group_totals["amount"], number_format)
                sheet.write_number(row, 8, group_totals["to_invoice"], number_format)
                sheet.write_number(row, 9, group_totals["invoiced"], number_format)
                row += 2
            ...
            for section in data:
                ...
                sheet.write_number(row, 3, section["total_amount"], number_format)
                sheet.write_number(
                    row, 4, section["total_to_invoice_qty"], number_format
                )
                sheet.write_number(row, 5, section["total_invoiced_qty"], number_format)
                ...
                grand_totals["to_invoice"] += section["total_to_invoice_qty"]
        ...
        sheet.write_number(
            row, col_offset + 4, grand_totals["to_invoice"], number_format
        )
        sheet.write_number(row, col_offset + 5, grand_totals["invoiced"], number_format)
